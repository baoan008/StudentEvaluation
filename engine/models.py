...
class Student(models.Model):
    ...
    theclass = models.ForeignKey(Class, verbose_name=u"班级")
...
class Assessment(models.Model):
    ...
    def __unicode__(self):
        return self.term + u'-互评设置'

    class Meta:
        db_table = u'assessment'
        verbose_name_plural = u'互评设置'

class AssessmentRecord(models.Model):
    assessment = models.ForeignKey(Assessment, verbose_name=u'互评')
    ostudent = models.ForeignKey(Student, verbose_name=u'评价同学', related_name='ostudent')
    dstudent = models.ForeignKey(Student, verbose_name=u'被评价同学', related_name='dstudent')
    ...
    class Meta:
        ...
        verbose_name_plural = u'互评记录'

class AssessmentRow(models.Model):
    assessment = models.ForeignKey(Assessment, verbose_name=u'互评')
    student = models.ForeignKey(Student, verbose_name=u'被评价同学')
    excellent = models.IntegerField(u'优')
    good = models.IntegerField(u'良')
    ordinary = models.IntegerField(u'中')
    ...
    class Meta:
        ...
        verbose_name_plural = u'每人互评记录'

class Grade(models.Model):
    term = models.CharField(u'学期', max_length=16)
    student = models.ForeignKey(Student, verbose_name=u'同学')
    score = models.FloatField(u'分数')

    def __unicode__(self):
        return self.term + "  " + self.student.realname + '-->' + str(self.score)
...
class Comperformance(models.Model):
    ...
    behavior = models.FloatField(u'日常行为基础分')
    development = models.FloatField(u'单项最高分')
    moral = models.FloatField(u'互评最高分')
    behaviorup = models.FloatField(u'日常行为最高分')
    term = models.CharField(u'学期', unique=True, max_length=16)

    def __unicode__(self):
        return self.term + u'-成绩设置'

    class Meta:
        db_table = 'comperformance'
        verbose_name_plural = u'综合测评设置'

class ComperformanceDevelopmentScore(models.Model):
    student = models.ForeignKey(Student, verbose_name=u"同学")
    comperformance = models.ForeignKey(Comperformance, verbose_name=u"综合成绩管理")
    development = models.ForeignKey(Development, verbose_name=u"个性发展")
    ...
    class Meta:
        ...
        verbose_name_plural = u'个性发展加分'

class ComperformanceBehaviorScore(models.Model):
    student = models.ForeignKey(Student,verbose_name=u"同学")
    comperformance = models.ForeignKey(Comperformance,verbose_name=u"综合成绩管理")
    behavior = models.ForeignKey(Behavior,verbose_name=u"日常行为")
    score = models.FloatField(u"分数",null=True, blank=True)
