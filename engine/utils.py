...
def get_datatables_records(request, querySet, columnIndexNameMap, jsonTemplatePath=None,
                           extrafilters={}, handler_user=False,
                           updatefilter={}, updateitems={}, *args):

    ######################################参数解析过程################################################
    cols = int(request.GET.get('iColumns', 0)) #获取多少列数据
    iDisplayLength = min(int(request.GET.get('iDisplayLength', 10)), 100) #没页获取rows个数
    startRecord = int(request.GET.get('iDisplayStart', 0)) #本页第一条数据是所有数据的第几个，下标从0开始
    ...
    #Ordering data
    iSortingCols = int(request.GET.get('iSortingCols', 0)) #根据columns进行排序
    asortingCols = []

    if iSortingCols:
        for sortedColIndex in range(0, iSortingCols):
            sortedColID = int(request.GET.get('iSortCol_'+str(sortedColIndex), 0))
            if request.GET.get('bSortable_{0}'.format(sortedColID), 'false') == 'true': #判断是否可以排序
                sortedColName = columnIndexNameMap[sortedColID]
                if handler_user: #根据用户realname进行排序
    ...
    if customSearch != '':
        ...
        #=========================handler user or filter========================
        if handler_user:
            kwargz = {'user_realname__icontains':customSearch}
